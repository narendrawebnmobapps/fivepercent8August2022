        ...
        swal({
            ...
            if (willDelete) 
            {
              $.ajax({
                  ...
                  {
                      swal("Done!", "Your stock Deleted Successfully", "success");
                      setTimeout(function () {
                          location.reload();
                      }, 1000);
                  },
           ...
           {
            swal("The Stock will Available in your portfolio");
           }
        });
}  
</script>
...
$(document).ready(function(){
    ...
    $('.percentage-filter-boxs1 .balancesheettest').on('click', 'li', function() {
            var id = $(this).attr('data-id');
            var text = '';
            if(id=="volatility")
            {
                text = "volatility (Yearly)";
            }
            else
            {
                text = id;
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    ...
                    var html = '';
                    html+='<tr><th><th></th></th><th>Company</th><th>No. Of Stock</th><th>Price</th><th>'+text+'</th><th></th><th></th></tr> ';
                    $.each(parse, function (index, value){
                        var result = $.base64.encode(value.id);
                        result = $.base64.encode(result);
                        var stock_id_enc = $.base64.encode(value.stock_id);
                        stock_id_enc = $.base64.encode(stock_id_enc);
                        var href = "<?php echo base_url('users/portfolio/details_stock_portfolio/') ?>"+result+"/"+stock_id_enc;
                        //console.log(result);
                        html+='<tr><td class="hide-row"><i style="cursor: pointer;" dataid="'+value.id+'" class="fa fa-minus delete-user-stock" onclick="confirmDelete('+value.id+')" aria-hidden="true"></i></td><td style="cursor: pointer;" class="esqmation" dataid="'+value.id+'"><i class="fa fa-info" aria-hidden="true"></i></td><td><a href="'+href+'">'+value.stock_name+'</a></td><td>'+value.number+'</td><td>'+value.price+'</td><td>'+value.volatility+'</td></tr>';
                    });
...
var stock_name = $('#add_stcok_form .stock_name').val();
var stock_type = $('#add_stcok_form .stock_type').val();
var number = $('#add_stcok_form .numberOfStock').val();
var ok=1;
if(!checkSelectBoxSelected('#add_stcok_form .stock_name'))
{
    ok=2;
}
/*if(!checkSelectBoxSelected('#add_stcok_form .stock_type'))
{
    ok=2;
}
if(!checkSelectBoxSelected('#add_stcok_form .stockPerPrice'))
{
    ok=2;
}
...
$.ajax({
        ...
        success:function(data)
        {
          ...
          if(data==1)
          {
            swal('Added','Stock has been added Successfully.','success');
            setTimeout(function () {
                location.reload();
            }, 1000);
          }
          ...
          {
            swal('Already','This stock is already in your portfolio.','warning');
            setTimeout(function () {
                location.reload();
            }, 1000);
          }
...
//Add option Stock
$(document).on('click','#add_option_stock_btn',function(event){
event.preventDefault();
var stock_name = $('#add_option_stcok_form .stock_name').val();
var stock_price = $('#add_option_stcok_form .stock_price').val();
var stock_type = $('#add_option_stcok_form .stock_type').val();
var number = $('#add_option_stcok_form .numberOfStock').val();
var ok=1;
if(!checkSelectBoxSelected('#add_option_stcok_form .stock_name'))
{
    ok=2;
}
/*if(!checkSelectBoxSelected('#add_option_stcok_form .stock_price'))
{
    ok=2;
}
if(!checkSelectBoxSelected('#add_option_stcok_form .stock_type'))
{
    ok=2;
}
if(!checkSelectBoxSelected('#add_option_stcok_form .numberOfStock'))
{
    ok=2;
}*/
...
{
$.ajax({
        url:'<?php echo base_url("users/portfolio/add_option_stock_ajax"); ?>',
        method:'POST',
        data:new FormData( $("#add_option_stcok_form")[0]),
        async : false,
        cache : false,
        contentType : false,
        processData : false,
        success:function(data)
        {
          ...
          {
            swal({
                  title: 'Exceeding Suggestion Limit',
                  text: "You are Exceeding our Suggestion limit. We suggested you to add "+json['canSpendPercent']+"% of amount in Option Stock",
                  icon: "warning",
                  buttons: true,
                  dangerMode: true,
              }).then((willDelete) => {
                  if (willDelete) 
                  {
                    $.ajax({
                        type:'POST',
                        url:'<?php echo base_url("users/portfolio/add_option_stock_against_suggestions_ajax"); ?>',
                        data:new FormData( $("#add_option_stcok_form")[0]),
                        async : false,
                        cache : false,
                        contentType : false,
                        ...
                        success: function (data) 
                        {
                            ...
                            if(json['res']==1)
                            {
                              swal('Added','Option has been added Successfully.','success');
                               setTimeout(function () {
                                    location.reload();
                                }, 1000);
                            }
                            ...
                            {
                              swal('Alert','You are already invested almost your limit of amount. You have only remaining amount $'+json['remaingInvest'] +' to invest','warning');
                            }
                            else
                            {
                 ...
                 {
                  swal("Alright! You are in safe mode of investment");
                 }
              });
           }
           else if(json['res']==3)
...
$.ajax({
        url:"<?php echo base_url('users/portfolio/show_stock_info_ajax'); ?>",
        method:'POST',
        data:{id:id},
        dataType:'html',
        success:function(data)
...
$(document).on('click','.edit-user-stock',function(){
    ...
    var stock_id = $(this).attr('dataid');
    $.ajax({
        url:"<?php echo base_url('users/portfolio/edit_stock'); ?>",
        method:'POST',
        data:{stock_id:stock_id},
        dataType:'html',
        success:function(data)
        {
          ...
          else
          {
            ...
            $('#edit_stcok_form .user_stock_id').val(value.id);
            $('#edit_stcok_form .stock_name').val(value.stock_id);
            $('#edit_stcok_form .stock_type').val(value.order_type);
            $('#edit_stcok_form .stockPerPrice').val(value.stock_price);
            $('#edit_stcok_form .numberOfStock').val(value.number);
...
//edit Option Stock
$(document).on('click','.edit-user-option-stock',function(){
    $('.loader-image').show();
    $('body').addClass('loader');
    var stock_id = $(this).attr('dataid');
    $.ajax({
        url:"<?php echo base_url('users/portfolio/edit_option_stock'); ?>",
        method:'POST',
        data:{stock_id:stock_id},
        dataType:'html',
        success:function(data)
        {
          ...
          {
            var value = JSON.parse(data);
            $('#edit_option_stcok_form .user_stock_id').val(value.id);
            $('#edit_option_stcok_form .stock_name').val(value.stock_name);
            $('#edit_option_stcok_form .stock_price').val(value.stock_price);
            $('#edit_option_stcok_form .stock_type').val(value.order_type);
            $('#edit_option_stcok_form .numberOfStock').val(value.number);
            $('#edit_stock_modal').modal('show');
          }
        }
    });    
  });
// end edit option stock
$(document).on('click','#update_stock_btn',function(event){
event.preventDefault();
var ok=1;
if(!checkSelectBoxSelected('#edit_stcok_form .stock_name'))
{
    ok=2;
}
/*if(!checkSelectBoxSelected('#edit_stcok_form .stock_type'))
{
    ok=2;
}
if(!checkSelectBoxSelected('#edit_stcok_form .stockPerPrice'))
{
    ok=2;
}
...
{
$.ajax({
        url:'<?php echo base_url("users/portfolio/update_stock_ajax"); ?>',
        method:'POST',
        ...
        success:function(data)
        {
          ...
          else if(json['res']==2)
          {
            swal({
                  title: 'Exceeding Suggestion Limit',
                  text: "You are Exceeding our Suggestion limit. We suggested you to add "+json['canSpendPercent']+"% of amount in stock",
                  icon: "warning",
                  buttons: true,
                  dangerMode: true,
              }).then((willDelete) => {
                  if (willDelete) 
                  {
                    $.ajax({
                        type:'POST',
                        url:'<?php echo base_url("users/portfolio/update_stock_with_against_suggestions_ajax"); ?>',
                        data:new FormData( $("#edit_stcok_form")[0]),
                        async : false,
                        cache : false,
                        contentType : false,
                        ...
                        success: function (data) 
                        {
                            ...
                            if(json['res']==1)
                            {
                              swal('Added','Stock has been Updated Successfully.','success');
                               setTimeout(function () {
                                    location.reload();
                                }, 1000);
                            }
...
//update option stock
$(document).on('click','#update_option_stock_btn',function(event){
event.preventDefault();
var ok=1;
if(!checkSelectBoxSelected('#edit_option_stcok_form .stock_name'))
{
    ok=2;
}
/*if(!checkSelectBoxSelected('#edit_option_stcok_form .stock_price'))
{
    ok=2;
}
if(!checkSelectBoxSelected('#edit_option_stcok_form .stock_type'))
{
    ok=2;
}
if(!checkSelectBoxSelected('#edit_option_stcok_form .numberOfStock'))
{
    ok=2;
}*/
...
{
$.ajax({
        url:'<?php echo base_url("users/portfolio/update_option_stock_ajax"); ?>',
        method:'POST',
        data:new FormData( $("#edit_option_stcok_form")[0]),
        async : false,
        cache : false,
        contentType : false,
        processData : false,
        success:function(data)
        {
          ...
          else if(data==1)
          {
           ...
           {
            //alert('You are already cross investment criteria');
            showSweetAlert("Alert",'You are already cross investment criteria');
           /* setInterval(function()
                    {  
                        location.reload();
                    }, 3000);*/
