...
{
	function file_handling($stock_file)
	{

			$the_big_array = [];
			$dataArr = array();
			$handle = fopen('uploads/stock/'.$stock_file, 'r');
			$c = 0;//
			while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
			{
				...
				if($c!=0)
				{		
					$the_big_array[] = str_replace(",",".",trim($filesop[6],'%'));
				}
				$c++;
			}
	...
	function get_price_file_handling($stock_file)
	{
	...
	*/
	...
	function calculateCustomVariance($arrays)
	{
		$variances[0]  = 0;
	    $c = 0;
	    foreach($arrays as $variance)
	    {
