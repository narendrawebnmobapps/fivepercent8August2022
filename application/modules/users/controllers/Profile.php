...
class Profile extends CI_Controller 
{
				...
				if($_FILES['proimage']['size']>0)
			     {
			      ...
			      else
			      {
					...
					$getCountryName = $this->db->query("SELECT name FROM countries WHERE id='".$country."'")->row();
					$this->session->set_userdata('country',$getCountryName->name);
	...
	function settings()
	{
		/*$testdata = file_get_contents('includes/8.html');
		//echo $testdata;

		$html = 'akhs akjhgsdkjhgk<!--UPLOADBUTTON-->
					    <button data-toggle="modal" data-target="#OpenImgUploadfootercompanylogo" class="btnUploadFlyer" style="background: crimson;padding: 5px 5px;color: #fff;border: none;cursor: pointer;font-size: 14px;outline: none;">Upload Image</button><!--UPLOADBUTTON-->askyd akys kdaks
					    <!DOCTYPE html>
					<html>
					<head>
					...
					<div class="container">
					  <div id="wrapper">
					    ...
					    <div id="desc">Lorem Ipsum is simply dummy text of the printing and typesetting industry.
					    	<!--UPLOADBUTTON--><button data-toggle="modal" data-target="#OpenImgUploadfootercompanylogo" class="btnUploadFlyer" style="background: crimson;padding: 5px 5px;color: #fff;border: none;cursor: pointer;font-size: 14px;outline: none;">Upload Image</button><!--UPLOADBUTTON-->akjs kahgsk akjgskj jai sri ram</div>
					    <!--UPLOADBUTTON-->
					    Jai bajrang bali
					    <button data-toggle="modal" data-target="#OpenImgUploadfootercompanylogo" class="btnUploadFlyer" style="background: crimson;padding: 5px 5px;color: #fff;border: none;cursor: pointer;font-size: 14px;outline: none;">Upload Image</button><!--UPLOADBUTTON-->jai hanuman ji
					  </div>
					  </div>

					</body>
					</html>';
		$epl = explode('<!--UPLOADBUTTON-->', $testdata);
		//echo count($epl);
		$iii = 0;
		$testhtml = '';
		foreach($epl as $val)
		...
		//echo "<pre>";print_r($epl);
		echo $testhtml;
		//$html = preg_replace('#<h1 class="ttl">Welcome to CodexWorld</h1>#', '', $html);
		//echo preg_replace('/display\h*:\h*none\h*;?/', '', $html);
		//echo $html;
		die;*/
	...
	function risk_profile()
	{
		...
		if($checkRisk==1)
		{
			$data['stock_rfs'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND a.volatility<'".$getRisksData->endPoint."' ORDER BY a.volatility  LIMIT 10")->result();
			$data['stock_rvs'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND a.volatility<'".$getRisksData->endPoint."' ORDER BY a.volatility  LIMIT 10")->result();
			$data['stock_options'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND a.volatility<'".$getRisksData->endPoint."' ORDER BY a.volatility DESC LIMIT 10")->result();
		}
		if($checkRisk==2)
		{
			$data['stock_rfs'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND  (a.volatility BETWEEN '".$getRisksData->startPoint."' AND '".$getRisksData->endPoint."') ORDER BY a.volatility  LIMIT 10")->result();
			$data['stock_rvs'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND  (a.volatility BETWEEN '".$getRisksData->startPoint."' AND '".$getRisksData->endPoint."') ORDER BY a.volatility  LIMIT 10")->result();
			$data['stock_options'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND  (a.volatility BETWEEN '".$getRisksData->startPoint."' AND '".$getRisksData->endPoint."') ORDER BY a.volatility DESC LIMIT 10")->result();
		}
		if($checkRisk==3)
		{
			$data['stock_rfs'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND  (a.volatility>'".$getRisksData->startPoint."' AND a.volatility<'".$getRisksData->endPoint."') ORDER BY a.volatility  LIMIT 10")->result();

			$data['stock_rvs'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND (a.volatility>'".$getRisksData->startPoint."' AND a.volatility<'".$getRisksData->endPoint."') ORDER BY a.volatility  LIMIT 10")->result();

			$data['stock_options'] = $this->db->query("SELECT a.id,a.stock_name,a.volatility,($sql1) as exist  FROM tbl_admin_stock a  WHERE a.stock_type=1 AND (a.volatility>'".$getRisksData->startPoint."' AND a.volatility<'".$getRisksData->endPoint."') ORDER BY a.volatility DESC LIMIT 10")->result();
		}
		


		//echo "<pre>";print_r($data['stock_rvs']);die;
		$plan_id = $this->session->userdata('plan_id');
		//echo $plan_id;die;
		if($plan_id==1 OR $plan_id==0 )
		{
			$this->load->view('page/profile/basic_risk_profile',$data);
		}
		else
		{
